...
void coupling_3(vector<miser> &mb ,vector<generous> &genb,vector<geek> &gb,vector<choosy> &g1 ,vector<normal> &g2,vector <desperate> &g3){
    int b_size = mb.size() + gb.size() + genb.size();
    ifstream input,input2;
    input.open("girls.txt", std::ios::in);
    input2.open("boys.txt", std::ios::in);
    for(int i = 0; i <b_size;i++ ){
		...
		input2 >> name >> attract  >> intelli  >> budget  >> min_attr_req  >> type ;
		//cout<< name <<" " << attract <<" "<< intelli <<" "<< budget<<" "<< min_attr_req <<" "<< type<<endl;
        if(name == "***")
            break;

        vector<choosy>::iterator it1,temp1;
        vector<normal>::iterator it2,temp2;
        vector<desperate>::iterator it3,temp3;
        //cout<<"AA\n";
        int max_att = INT_MIN;
        int t = 0;
        for(it1 = g1.begin();it1!=g1.end();it1++){
            if(it1->get_status() == 0 && it1->get_maintanence() > budget){
                temp1 = it1;
                if(temp1->get_attractiveness() > max_att){
                    max_att = temp1->get_attractiveness();
                    t = 1;
                }
                break;
            }
        ...
        for(it2 = g2.begin();it2!=g2.end();it2++){
            if(it2->get_status() == 0 && it2->get_maintanence() > budget){
                temp2 = it2;
                if(temp2->get_attractiveness() > max_att){
                    max_att = temp2->get_attractiveness();
                    t = 2;
                }
        ...
        for(it3 = g3.begin();it3!=g3.end();it3++){
            if(it3->get_status() == 0 && it3->get_maintanence() > budget){
                temp3 = it3;
                if(temp3->get_attractiveness() > max_att){
                    max_att = temp3->get_attractiveness();
                    t = 3;
                }
        ...
        //cout <<temp1->get_attractiveness()<<endl;
            if(t == 1){
                cout << name<<" with  "<<temp1->get_name()<<endl;
                temp1->set_status(1);

        }
        }
        else if(temp2 != g2.end()){
                //cout <<"2\n";
            if(t == 2){
                cout << name<<" with  "<<temp2->get_name()<<endl;
                temp2->set_status(1);

           }
        }
        else if(temp3 != g3.end()){
                //cout <<"3\n";
            if(t == 3){
                cout << name<<" with  "<<temp3->get_name()<<endl;
                temp3->set_status(1);
